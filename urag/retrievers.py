...
class HybridRetriever():
    def __init__(
        ...
        self.retrieval_type = config["retrieval_type"]  # 1:dense only 2:sparse only 3:hybrid
        self.filters = None
        self.filter_dict = None
        self.topk = topk
        self.config = config
    ...
    def retrieve(self, query_bundle: QueryBundle) -> List[Document]:
        ...
        dense_nodes = RetrievalService.retrieve(query=query_bundle.query_str,config=self.config)
        sparse_nodes = self.sparse_retriever.retrieve(query_bundle)

        # combine the two lists of nodes,去重
        all_nodes = []
        node_ids = set()
        for n in sparse_nodes + dense_nodes:
            if n.metadata['id_'] not in node_ids:
                all_nodes.append(n)
                node_ids.add(n.metadata['id_'])
        return all_nodes
