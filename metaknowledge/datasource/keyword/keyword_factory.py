...
from metaknowledge.datasource.keyword.keyword_type import KeyWordType
...
class Keyword:
    ...
    def _init_keyword(self) -> BaseKeyword:
        keyword_type = self.config['keyword_store']
        keyword_factory = self.get_keyword_factory(keyword_type)
        return keyword_factory(self.config)

    @staticmethod
    def get_keyword_factory(keyword_type: str) -> type[BaseKeyword]:
        match keyword_type:
            case KeyWordType.JIEBA:
                from metaknowledge.datasource.keyword.jieba.jieba import Jieba
