...
def threads_semaphore_do_work(execute_function,semaphore,thread_name,**kwargs):
    with semaphore:
...
def check_if_file_or_dir(path)->Union[List,str]:
    if os.path.isfile(path):
        return path
    elif os.path.isdir(path):
        return [os.path.join(path, f) for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]
    else:
        return loguru.logger.info(f"File or directory not found: %s" % path)
